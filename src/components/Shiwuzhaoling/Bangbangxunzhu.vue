...
export default {
  ...
  watch: {
    xunzhuPage: function(page) {
      this.$store.commit("xunzhuPage", page);
      this.getData();
    },
    xunzhuPageSize: function(pageSize) {
      this.$store.commit("xunzhuPageSize", pageSize);
      this.getData();
    },
  ...
  methods: {
    async getData() {
      const res = await this.$api.getshougou({
        limit: this.xunzhuPageSize,
        page: this.xunzhuPage,
        type: 2
      });
    ...
    handleSizeChange(val) {
      ...
      this.$store.commit("xunzhuPageSize", val);
    },
    handleCurrentChange(val) {
      console.log(`当前页: ${val}`);
      this.$store.commit("xunzhuPage", val);
    }
  }
};
</script>
