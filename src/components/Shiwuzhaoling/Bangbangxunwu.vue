    ...
    <div class="nav2">
      ...
      <div class="myTable">
        <vxe-table :data="tableData">
          ...
          <vxe-table-column field="product_name" title="物品"></vxe-table-column>
          ...
          <vxe-table-column title="操作状态" width="140">
            <template slot-scope="scope">
              <div class="flex">
                ...
                <!-- <el-button size="small" @click="toDelShop(scope.row)" type="text">审核</el-button> -->
...
export default {
  computed: {
    ...mapState(["xunwuPage", "xunwuPageSize",
      "biaobaiqiangPinglunPage",
      "biaobaiqiangPinglunPageSize"])
  },
  watch: {
    xunwuPage: function(page) {
      this.$store.commit("xunwuPage", page);
      this.getData();
    },
    xunwuPageSize: function(pageSize) {
      this.$store.commit("xunwuPageSize", pageSize);
      this.getData();
    },
  ...
  methods: {
    ...
    handleSizeChange(val) {
      ...
      this.$store.commit("xunwuPageSize", val);
    },
    handleCurrentChange(val) {
      console.log(`当前页: ${val}`);
      this.$store.commit("xunwuPage", val);
    }
  }
};
</script>
