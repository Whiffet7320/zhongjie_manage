    ...
    <div class="nav2">
      ...
      <div class="myTable">
        <vxe-table :data="tableData">
            ...
            <template #content="{ row }">
              <template>
                <div class="xiala">
                  <el-row :gutter="20">
                    <el-col :span="18">
                      <div class="item">树洞内容：{{row.content}}</div>
                    </el-col>
                  </el-row>
          ...
          <vxe-table-column field="myIs_best" width="120" title="是否精选">
            <template slot-scope="scope">
              <el-switch @change="changeJX(scope.row)" v-model="scope.row.myIs_best"></el-switch>
            </template>
          </vxe-table-column>
...
export default {
  ...
  watch: {
    ...
    shudongPageSize: function(pageSize) {
      this.$store.commit("shudongPageSize", pageSize);
      this.getData();
    },
  ...
  async created() {
    // const res = await this.$api.upload_video();
    // console.log(res)
  ...
  methods: {
    async getData() {
      ...
      this.tableData.forEach(ele => {
        ...
        ele.myIs_best = ele.is_best == "1" ? true : false;
        console.log(ele.myIs_best);
        if (ele.img_paths) {
    ...
    async changeJX(row) {
      console.log(row);
      const res = await this.$api.is_best({
        is_best: row.myIs_best == true ? "1" : "0",
        id: row.id
      });
      if (res.code == 200) {
        this.$message({
    ...
    handleSizeChange(val) {
      ...
      this.$store.commit("shudongPageSize", val);
    },
    handleCurrentChange(val) {
      console.log(`当前页: ${val}`);
      this.$store.commit("shudongPage", val);
    }
  }
};
</script>
