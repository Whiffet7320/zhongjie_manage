<template>
  <div class="index">
    ...
    <div class="nav2">
      ...
      <div class="tit1">
        <el-button @click="AddFenlei" size="small" type="primary" icon="el-icon-plus">添加关键词</el-button>
      </div>
      <div class="myTable">
        <vxe-table
          :tree-config="{
        ...
        >
          ...
          <vxe-table-column tree-node field="search_key" title="关键词"></vxe-table-column>
          <vxe-table-column tree-node field="add_time" title="创建时间"></vxe-table-column>
          ...
          <el-row>
            <el-col :span="20">
              <el-form-item label="关键词">
                <el-input size="small" placeholder="请输入关键词" v-model="addForm.category_name"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
...
export default {
  ...
  methods: {
    async getData() {
      const res = await this.$api.search_key({
    ...
    tabEdit(row) {
      ...
      this.isAdd = false;
      this.id = row.id;
      this.addForm.category_name = row.search_key;
      this.addDialogVisible = true;
    },
    async tabDel(row) {
      console.log(row);
      const res = await this.$api.delSearch_key(row.id);
      if (res.code == 200) {
        this.$message({
          message: res.message,
          type: "success"
    ...
    async AddOnSubmit() {
      ...
      if (this.isAdd) {
        //   添加
        const res = await this.$api.addSearch_key({
          search_key:this.addForm.category_name
        });
        if (res.code == 200) {
          this.$message({
            message: res.message,
        ...
        const res = await this.$api.upDateSearch_key(
          {
            search_key:this.addForm.category_name
          },
          this.id
