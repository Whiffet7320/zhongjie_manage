<template>
  <div class="index">
    ...
    <div class="nav2">
      ...
      <div class="tit1">
        <el-button @click="AddFenlei" size="small" type="primary" icon="el-icon-plus">添加用户等级</el-button>
      </div>
      <div class="myTable">
        <vxe-table
          :tree-config="{
        ...
        >
          ...
          <vxe-table-column field="level_name" title="等级名称"></vxe-table-column>
          ...
          <el-row>
            <el-col :span="20">
              <el-form-item label="等级">
                <el-input size="small" placeholder="请输入等级" v-model="addForm.level"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="20">
              <el-form-item label="等级名称">
                <el-input size="small" placeholder="请输入等级名称" v-model="addForm.level_name"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
...
export default {
  ...
  methods: {
    async getData() {
      const res = await this.$api.user_level({
    ...
    async tabDel(row) {
      console.log(row);
      const res = await this.$api.deluser_level(row.id);
      if (res.code == 200) {
        this.$message({
          message: res.message,
          type: "success"
    ...
    async AddOnSubmit() {
      ...
      if (this.isAdd) {
        //   添加
        const res = await this.$api.Adduser_level({
          level_name: this.addForm.level_name,
        ...
        const res = await this.$api.upDateuser_level(
          {
            level_name: this.addForm.level_name,
            pid: this.addForm.pid,
            level: this.addForm.level
