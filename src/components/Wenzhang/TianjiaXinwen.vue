...
export default {
  ...
  methods: {
    ...
    async onSubmitFormcyy() {
      ...
      if (this.xinwenObj) {
        // 修改
        const res = await this.$api.upDateNews(
          {
        ...
        if (res.code == 200) {
          ...
          this.$router.push({name:'Xinwenliebiao'})
        } else {
          this.$message.error(res.msg);
          this.getData();
        }
      }else {
        const res = await this.$api.addNews(
          {
            title: this.lhForm.title,
            content: this.lhForm.content,
            img: this.lhForm.img,
