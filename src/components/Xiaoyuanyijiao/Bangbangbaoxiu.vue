        ...
        <vxe-table :data="tableData">
          ...
          <vxe-table-column field="avatar_url" title="发布者头像">
            <template slot-scope="scope">
              <el-image :preview-src-list="[scope.row.avatar_url]" :src="scope.row.avatar_url" fit="fill" style="width: 40px; height: 40px">
                <div slot="error" class="image-slot">
                  <i class="el-icon-picture-outline"></i>
                </div>
              </el-image>
          ...
          <vxe-table-column field="product_name" title="报修品"></vxe-table-column>
          <!-- <vxe-table-column field="price" title="价格(元)"></vxe-table-column> -->
...
export default {
  computed: {
    ...mapState(["baoxiuPage", "baoxiuPageSize",
      "biaobaiqiangPinglunPage",
      "biaobaiqiangPinglunPageSize"])
  },
  watch: {
    baoxiuPage: function(page) {
      this.$store.commit("baoxiuPage", page);
      this.getData();
    },
    baoxiuPageSize: function(pageSize) {
      this.$store.commit("baoxiuPageSize", pageSize);
      this.getData();
    },
  ...
  methods: {
    ...
    handleSizeChange(val) {
      ...
      this.$store.commit("baoxiuPageSize", val);
    },
    handleCurrentChange(val) {
      console.log(`当前页: ${val}`);
      this.$store.commit("baoxiuPage", val);
    }
  }
};
</script>
