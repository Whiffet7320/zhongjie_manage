    ...
    <div class="nav2">
      ...
      <div class="myTable">
        ...
        <el-pagination
          ...
          :current-page="this.fengjingPage"
          :page-size="10"
...
export default {
  computed: {
    ...mapState(["fengjingPage", "fengjingPageSize",
      "biaobaiqiangPinglunPage",
      "biaobaiqiangPinglunPageSize"])
  },
  watch: {
    fengjingPage: function(page) {
      this.$store.commit("fengjingPage", page);
      this.getData();
    },
    fengjingPageSize: function(pageSize) {
      this.$store.commit("fengjingPageSize", pageSize);
      this.getData();
    },
  ...
  methods: {
    async getData() {
      const res = await this.$api.getfengjing({
        limit: this.fengjingPageSize,
        page: this.fengjingPage
      });
      ...
      this.tableData.forEach(ele => {
        ele.mySender_name =
          ele.sender_name == "" || !ele.sender_name
            ? "匿名用户"
            : ele.sender_name;
    ...
    handleSizeChange(val) {
      ...
      this.$store.commit("fengjingPageSize", val);
    },
    handleCurrentChange(val) {
      console.log(`当前页: ${val}`);
      this.$store.commit("fengjingPage", val);
    }
  }
};
</script>
