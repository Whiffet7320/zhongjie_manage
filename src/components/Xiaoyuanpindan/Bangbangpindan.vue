    ...
    <div class="nav2">
      ...
      <div class="myTable">
        <vxe-table :data="tableData">
          ...
          <vxe-table-column field="product_name" title="拼单物品"></vxe-table-column>
          <vxe-table-column field="price" title="拼单价格(元)"></vxe-table-column>
          <vxe-table-column field="people" title="拼单人数"></vxe-table-column>
...
export default {
  ...
  watch: {
    pindanPage: function(page) {
      this.$store.commit("pindanPage", page);
      this.getData();
    },
    pindanPageSize: function(pageSize) {
      this.$store.commit("pindanPageSize", pageSize);
      this.getData();
    },
  ...
  methods: {
    ...
    handleSizeChange(val) {
      ...
      this.$store.commit("pindanPageSize", val);
    },
    handleCurrentChange(val) {
      console.log(`当前页: ${val}`);
      this.$store.commit("pindanPage", val);
    }
  }
};
</script>
