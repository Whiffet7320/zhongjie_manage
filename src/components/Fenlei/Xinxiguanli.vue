<template>
  <div class="index">
    ...
    <div class="nav2">
      ...
      <div class="tit1">
        <el-button @click="AddFenlei" size="small" type="primary" icon="el-icon-plus">添加</el-button>
      </div>
      <div class="myTable">
        <vxe-table
          :tree-config="{
        ...
        >
          ...
          <vxe-table-column field="icon_url" title="图片">
          ...
          <vxe-table-column field="country_name" title="城市"></vxe-table-column>
          <vxe-table-column field="category_name" title="分类"></vxe-table-column>
          <vxe-table-column field="search_key" title="关键词"></vxe-table-column>
          <!-- <vxe-table-column field="fee_desc" title="费用介绍"></vxe-table-column>
          <vxe-table-column field="shop_desc" title="店铺介绍"></vxe-table-column>
          <vxe-table-column field="hangye_desc" title="行业介绍"></vxe-table-column> -->
          <vxe-table-column field="cc_desc" title="简介(中)"></vxe-table-column>
          <vxe-table-column field="english_cc_desc" title="简介(英)"></vxe-table-column>
...
export default {
  ...
  methods: {
    async getData() {
      const res = await this.$api.ccrelation({
      ...
      this.tableData.forEach(ele => {
        ele.myIcon_url = `${this.$url}/${ele.img}`;
      });
    },
    // 开关（显示/隐藏）
    async changeKG(row) {
    ...
    AddFenlei() {
      ...
      this.$router.push({name:'Tianjiaxinxi'})
    ...
    async tabDel(row) {
      console.log(row);
      const res = await this.$api.delCcrelation(row.id);
      if (res.code == 200) {
        this.$message({
          message: res.message,
          type: "success"
