    ...
    <div class="nav2">
      ...
      <div class="myTable">
        ...
        >
          ...
          <vxe-table-column field="shop_name" title="名称"></vxe-table-column>
          ...
          <vxe-table-column field="myCheck_status" title="状态"></vxe-table-column>
          ...
          <vxe-table-column title="操作" width="180">
            <template slot-scope="scope">
              <div class="flex">
                <el-button size="small" type="text" @click="tabEdit(scope.row)">通过</el-button>
                <el-button size="small" type="text" @click="tabDel(scope.row)">拒绝</el-button>
              </div>
            </template>
          </vxe-table-column>
        </vxe-table>
...
export default {
  ...
  methods: {
    async getData() {
      const res = await this.$api.country_server({
      ...
      this.tableData.forEach(ele => {
        ele.myCheck_status = ele.check_status == 0 ? '待审核' : ele.check_status == 1 ? '审核通过' : '审核不通过'
      });
    },
    // 开关（显示/隐藏）
    async changeKG(row) {
    ...
    async tabEdit(row) {
      console.log(row);
      const res = await this.$api.upDateCountry_server({
          check_status:1,
      },row.id)
      if (res.code == 200) {
        this.$message({
